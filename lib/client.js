...
var url = require("url");
var detectProxy = require("./proxy").detect;
...
function Client(options) {
  ...
  this.proxy = options.proxy || detectProxy(this.url);

  if (this.proxy) {
    this.proxy = url.parse(this.proxy);
    if (!this.proxy.hostname || !this.proxy.port) {
      debug("Could not parse proxy option, ignoring");
      this.proxy = null;
    }
  }
}
...
Client.prototype.request = function(method, url, query, callback) {
  ...
  req = protocol.request(options, function(res) {
    ...
    headers = res.headers;
    // browser doesn't support setEncoding
    if (res.setEncoding) {
